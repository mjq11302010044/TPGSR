...
class AttentionRecognitionHead(nn.Module):
  ...
  def beam_search(self, x, beam_width, eos, embed):
    ...
    def _inflate(tensor, times, dim):
      ...
      return tensor.repeat(*repeat_dims).to(device)
    ...
    sequence_scores.index_fill_(0, torch.Tensor([i * beam_width for i in range(0, batch_size)]).long(), 0.0).to(device)
    ...
    for i in range(self.max_len_labels):
      ...
      sequence_scores = _inflate(sequence_scores, self.num_classes, 1).to(device)
